...
<xsl:stylesheet version="1.0"
 ...
 >
	...
	<xsl:variable name="t-errordesc">Sorry, there was a problem displaying search results. No valid response was found. Contact washelp@ucop.edu for assistance.</xsl:variable>
	...
	<xsl:template match="atom:feed | rss/channel | rdf:RDF">
		...
		<html xml:lang="{$language}" lang="{$language}">
			...
			<head>
				...
				<link rel="stylesheet" href="css/opensearch_to_html.css" type="text/css" title="default" media="screen" />
				<!-- rel links -->
			...
			<body>
				...
				<xsl:choose>
					<xsl:when test="$items">
						...
						<!-- hack for xslt processors that do not respect disable-output-escaping="yes" --><xsl:if test="string-length($use-doe-hack)"><span id="encoding-test" style="display:none"><xsl:text disable-output-escaping="yes">&amp;amp;</xsl:text></span><script type="text/javascript" src="script/opensearch_to_html.js"></script></xsl:if>
						<!-- result navigation -->
						<p id="nav">
